...
// cleanupPRBodyForMerge removes the stack info footer, markdown comments, and empty template from PR body
func cleanupPRBodyForMerge(body string) string {
	...
	// remove HTML comments (both <!-- --> and <!--- ---> variants)
	// (?s) enables . to match newlines for multi-line comments
	htmlCommentRegex := regexp.MustCompile(`(?s)<!--.*?-->`)
	body = htmlCommentRegex.ReplaceAllString(body, "")

	// remove link reference style markdown comments
	// matches [//]: # (comment), []: # (comment), [comment]: # (comment), etc.
	// also handles variants with quotes
	linkCommentRegex := regexp.MustCompile(`(?m)^\[[\w/]*]:\s*#\s*[("'].*[)"']?\s*$`)
	body = linkCommentRegex.ReplaceAllString(body, "")

	// now remove stack info footer
	// look for --- separator that comes after an empty line and contains PR references
	// this preserves markdown headers with underlines (e.g., Test\n----)
	lines := strings.Split(body, "\n")
	footerStart := -1
	
	for i := 0; i < len(lines)-1; i++ {
		// check if next line is exactly "---"
		if strings.TrimSpace(lines[i]) == "---" {
			// check if there's at least one empty line before this separator
			...
			if hasEmptyLineBefore {
				// look ahead to see if this section contains stack info (PR references)
				hasStackInfo := false
				for j := i+1; j < len(lines); j++ {
					if matched, _ := regexp.MatchString(`^\*.*#\d+`, strings.TrimSpace(lines[j])); matched {
						hasStackInfo = true
						break
					}
				}
	...
	if footerStart >= 0 {
		// remove everything from the empty line before footer
		lines = lines[:footerStart]
		body = strings.Join(lines, "\n")
	}

	// clean up multiple consecutive blank lines left after removing comments
	multipleBlankLinesRegex := regexp.MustCompile(`\n{3,}`)
	body = multipleBlankLinesRegex.ReplaceAllString(body, "\n\n")

	// remove trailing <br> tags (with optional whitespace and newlines around them)
	// this removes patterns like: <br><br><br> or <br/> or <br /> at the end
	...
	// check if body only contains headers (markdown headers with underlines or # headers)
	// without any actual content
	trimmed := strings.TrimSpace(body)
	// match body that only has:
	// - headers with # (e.g., # Summary, ## Test)
	// - headers with underlines (e.g., Summary\n---, Test\n====)
